...
vec3 CDM(sampler2D perlin_img, vec2 pixel_coords);//central difference method
...
void main()
{
	...
	float v = pixel_coords.y/512.0f;//divide by 512 for a value between 1 and 0
    ...
    vec4 pixel = vec4(normals.r, normals.g, normals.b, perlin.y);//returns the height in the alpha and the normals in rgb
    imageStore(normals_output, pixel_coords, pixel);//stores the pixels to an image/texture
} 
...
vec3 CDM(sampler2D perlin_img, vec2 pixel_coords)
{
	...
	float bottomLeft = (textureOffset(perlin_img, pixel_coords, ivec2(-1, -1)).r) * scale;//added these for more accurate normals

	float centre1 = topLeft - bottomRight;
	float centre2 = topRight - bottomLeft;//diagnals

	float lr = left - right;
    float ud = up - down;//normal
